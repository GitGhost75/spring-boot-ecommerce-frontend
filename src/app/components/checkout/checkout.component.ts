...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // read users email from session storage
  ...
  onSubmit() {
    ...
    if (this.checkoutFormGroup.invalid) {
      // triggers displaying the error messages
  ...
  handleMonthsAndYears() {
    ...
    // if current year is selected year, start with current month
    ...
    this.luv2ShopFormService.getStates(countryCode).subscribe(
      data => {
        if (formGroupName === 'shippingAddress') {
          ...
          console.error('UNKNOWN FORM GROUP NAME ' + formGroupName);
        }
        // select first item
